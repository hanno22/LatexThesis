...
\begin{document}
	...
	\begin{figure}		
		...
		\begin{subfigure}[b]{0.49\textwidth}
			...
			\label{fig:fp_135}
		\end{subfigure}
		\caption{Die Abbildung zeigt die Anregung im Ortsraum an einem Punktdefekt. Der direkt transmittierte Strahl wurde dabei mit dem BB aus der Abbildung gefiltert. Abbildung \subref{fig:fp_45}} ist mit einer Orientierung der $\lambda / 4$-Platte von $\alpha_{\lambda/4} = 45^\circ$ aufgenommen und Abbildung  \subref{fig:fp_135} mit $\alpha_{\lambda/4} = 135^\circ$. Diese Orientierungen sorgen für eine links- bzw. rechts-zirkulare Polarisation des Anregungslasers. Der Laser fällt hierbei entlang der $x$-Achse aus $-x$-Richtung (ToDo!! Richtung in Polarkoordinaten beschreiben) auf den Punktdefekt ein. Das Anregungszentrum liegt im Ursprung des Koordinatensystems. Negative $x$-Werte entsprechen also einer Rückwärtspropagation und positiver Werte von $x$ entsprechen einer Vorwärtspropagation des SPP bezüglich der Einfallsrichtung des Anregungslasers.
		\label{fig:spin_hall_measure_fp}
	\end{figure}
	In den Rohdaten ist in Rückwärtsrichtung bezüglich der Einfallsrichtung des Anregungslasers bereits eine leichte Asymmetrie zu beobachten: Die Intensität der Anregung ist in Abbildung \ref{fig:fp_45} zwischen $90^\circ$ und $180^\circ$ größer, als zwischen $180^\circ$ und $270^\circ$. Bei Wechsel des Drehsinns der Polarisation des Anregungslaser (Abbildung \ref{fig:fp_135}) invertiert diese Asymmetrie ihr Vorzeichen. In Vorwärtsrichtung ist bemerkenswerterweise eine Asymmetrie zu beobachten, die zur Asymmetrie in Rückwärtsrichtung entgegengesetzt ist: In Abbildung \ref{fig:fp_45} ist zwischen die gemessene Intensität zwischen $0^\circ$ und $90^\circ$ größer als zwischen $270^\circ$ und $0^\circ$. Auch diese Asymmetrie invertiert bei Wechsel des Drehsinns der Polarisation ihr Vorzeichen. Um dieses Verhalten detaillierte zu untersuchen, wurde zunächst ein Differenzbild zwischen den Intensitäten, die bei links- bzw. rechts-zirkular Polarisierter Anregungsstrahlung gemessen worden errechnet. Hierfür wurde für jeden Bildpunkt die relative Differenz $D_ij$ zwischen der Intensität mit links- bzw. rechts-zirkular Polarisierter Anregungsstrahlung ($I_{\mathrm{lcp}, ij}$, $I_{\mathrm{rcp}, ij}$)errechnet. Die $x$-, $y$-Indizes des jeweiligen Pixels werden hierbei durch $i,j$ angegeben.
	\begin{equation}
		D_{ij} =\dfrac{I_{\mathrm{lcp}, ij} - I_{\mathrm{rcp}, ij}}{I_{\mathrm{lcp}, ij} + I_{\mathrm{rcp}, ij}}
		\label{eq:diff_measure}
	\end{equation}
	Das so entstanden Differenzbild ist in Abbildung \ref{fig:diff_fp_45_135} dargestellt. Um nun die Asymmetrie der Anregung in Abhängigkeit der Polarisation der Anregung darzustellen, wurden in dem Ortsraum zwei Bereiche ausgewählt, in denen die Intensität jeweils integriert wurde. Diese Integration  wurde für unterschiedliche Orientierungen des $\lambda/4$-Plättchens durchgeführt. Das Ergebnis dieser Integration ist in Abbildung \ref{fig:int_fp_45_135} für zwei in Rückwärtsrichtung orientierte Integrationsbereiche dargestellt.
	\begin{figure}		
		\begin{subfigure}[b]{0.5\textwidth}
			\centering
		...
		\begin{subfigure}[b]{0.49\textwidth}
			...
			\caption{Integrierte  Intensität in Abhängigkeit der Polarisation}
			\label{fig:int_fp_45_135}
		\end{subfigure}
		\caption{Abbildung \subref{fig:diff_fp_45_135} zeigt die nach \eqref{eq:diff_measure} berechnete Differenz der Intensitäten im Ortsraum bei links- bzw. rechts-zirkular Polarisierte Anregungsstrahlung. Abbildung \ref{fig:int_fp_45_135} zeigt die über die in \subref{fig:diff_fp_45_135} markierten Bereiche integrierte Intensität in Abhängigkeit der Orientierung des $\lambda/4$-Plättchens.}
