...
\begin{document}
	...
	Zum Nachweis des PSHE wurde mit Hilfe des Leckstrahlmikroskopes die Anregung eines SPPs an einem Punktdefekt in Abhängigkeit der Orientierung des $\lambda /4$-Plättchens untersucht. Es wurde für unterschiedliche Orientierungen des $\lambda /4$-Plättchens jeweils eine Intensitätsmessung der Leckstrahlung im Ortsraum und im Impulsraum durchgeführt. Die gemessene Intensitätsverteilung im Ortsraum werde durch $I_r(|\vec{r}|, \phi, \alpha_{\lambda /4})$ und die gemessene Intensitätsverteilung im Impulsraum durch $I_k(|\vec{k}_\perp|, \phi, \alpha_{\lambda /4})$ beschrieben (die Umrechnung der kartesischen Pixeldaten in Polarkoordinaten wird in Abschnitt \ref{sec:polar_calculation} näher erläutert). Hierbei ist $r$ der radiale Abstand zum Anregungszentrum, $|\vec{k}_\perp|$ der Betrag des Wellenvektor senkrecht zu optischen Achse und $\alpha_{\lambda/4}$ die Orientierung des $\lambda/4$-Plättchens. Der Azimutalwinkel $\phi$ beschreibt in der Ortsraumdarstellung den Winkel zu einem Ort und in der Impulsdarstellung eine Richtung des Wellenvektors. Da Koordinatensystem des Ortsraumes so gewählt worden ist, dass das Anregungszentrum im Ursprung liegt, entspricht der Winkel zu einem Punkt im Ortsraum auch gerade dem Winkel des Wellenvektors, der diesen Punkt im Ortsraum erreichen kann. $\phi = 0$ liegt hierbei in der Einfallsebene in Richtung der anregenden Strahlung. Das Ortsbild wurde in der ersten Bildebene zwischen Tubuslinse und $4f$-Aufbau mit der Blende (IB) auf einen einzelnen Defekt gefiltert. Der direkt Transmittierte Strahl wurde mit dem Beam-Block in der BFP1 gefiltert. Die BFP wurde für unterschiedliche Orientierungen des $\lambda/4$-Plättchens von $0^\circ-360^\circ$ mit einer Schrittweite von $2^\circ$ aufgenommen. Das Ortsbild der Anregung wurde mit einer Schrittweite von $20^\circ$ aufgezeichnet.
	...
	\end{figure}
	\paragraph{Ortsraum}
	In den Rohdaten ist im Ortsbild in Rückwärtsrichtung bezüglich der Einfallsrichtung des Anregungslasers bereits eine leichte Asymmetrie zu beobachten: Die Intensität der Anregung ist in Abbildung \ref{fig:raw_fp_45} zwischen $90^\circ$ und $180^\circ$ größer, als zwischen $180^\circ$ und $270^\circ$. Bei Wechsel des Drehsinns der Polarisation des Anregungslaser (Abbildung \ref{fig:raw_fp_135}) invertiert diese Asymmetrie ihr Vorzeichen. In Vorwärtsrichtung ist bemerkenswerterweise eine Asymmetrie zu beobachten, die zur Asymmetrie in Rückwärtsrichtung entgegengesetzt ist: In Abbildung \ref{fig:raw_fp_45} ist zwischen die gemessene Intensität zwischen $0^\circ$ und $90^\circ$ größer als zwischen $270^\circ$ und $0^\circ$. Auch diese Asymmetrie invertiert bei Wechsel des Drehsinns der Polarisation ihr Vorzeichen.
	...
	\subsubsection{Differenz- und Integrationsdaten}
		...
		\end{equation} 
		 ...
		 \end{align}
	 	Hierbei ist $r_\mathrm{max} = ...\mathrm{\mu m}$ die Begrenzung der Integrationsbereiche in radialer Richtung. Dieser Wert wurde gewählt, da für $|\vec{r}| > ...\mathrm{\mu m}$ nur noch geringe Intensitäten gemessen worden sind. Die Winkel $\phi_\mathrm{min} < \phi_\mathrm{max}$ legen die beiden Integrationsbereiche in azimutaler Richtung fest. Diese über eine Fläche integrierte Intensität ist eine Leistung. Die Integration wurde für alle Orientierungen des $\lambda/4$-Plättchens durchgeführt. Die Leistungen des oberen und unteren SPP wurde mit der Beziehung \eqref{eq:relative_power} auf die Summe der beiden Leistungen normiert. Das Ergebnis dieser Auswertung ist in den Abbildung \ref{fig:int_fp_45_135}, \subref{fig:int_fp_45_135_mid}, \subref{fig:int_fp_45_135_front} für unterschiedliche Kombinationen von $\phi_\mathrm{min}$und$\phi_\mathrm{max}$ dargestellt. Die Integrationsbereiche sind dabei jeweils in  \ref{fig:diff_fp_45_135}, \subref{fig:diff_fp_45_135_mid}, \subref{fig:diff_fp_45_135_front} markiert.
		  
		  (Diskussion) Dieser beobachtete Verlauf der Intensitäten zwischen oberen und unteren SPP in Rückwärtsrichtung zeigt bei zirkularer Polarisation ($\alpha_{\lambda /4} \in \{45^\circ, 135^\circ, 225^\circ, 315^\circ\}$) jeweils die mit dem Drehsinn der Polarisation wechselnde Asymmetrie.
		\begin{figure}		
			\begin{subfigure}[b]{0.5\textwidth}
				\centering
				\includegraphics[width=\textwidth]{figures/fp/fp_back.png}
				\caption{Relative Intensitäts Differenz}
			...
			\begin{subfigure}[b]{0.49\textwidth}
				...
				\caption{Integrierte  Intensität}
			...
			\end{subfigure}
			\caption{Die Abbildung \subref{fig:diff_fp_45_135}, \subref{fig:diff_fp_45_135_mid}, \subref{fig:diff_fp_45_135_front} zeigt die nach \eqref{eq:diff_measure} berechnete Differenz der Intensitäten im Ortsraum zwischen links- bzw. rechts-zirkular Polarisierte Anregungsstrahlung. Diese Abbildungen unterscheiden sich nur in den markierten Integrationsbereichen. Die Abbildung \ref{fig:int_fp_45_135}, \ref{fig:int_fp_45_135_mid}, \ref{fig:int_fp_45_135_front} zeigen jeweils die mit \eqref{eq:relative_power} über die zugehörigen markierten Bereiche integrierte Intensität normiert auf die Summe der Intensitäten beider Bereiche in Abhängigkeit der Orientierung des $\lambda/4$-Plättchens. }
		...
		\end{align}
		Da im Impulsraum von der Leckstrahlung nur ein scharfer Kreisring mit Radius $k = k_\mathrm{SPP}$ erwartet wird, wurde bei der Integration noch zusätzlich eine untere Integrationsgrenze $k_min$ in radialer Richtung eingeführt. Das Differenzbild und die Auswertung der Integrationsbereiche sind in Abbildung \ref{fig:spin_hall_measure} dargestellt.		
		\begin{figure}
		...
		\end{figure}
	\FloatBarrier
	\subsubsection{Azimutale Intensität und Intensitätsdifferenz}	
		Da für die unterschiedlichen Orientierung des Integrationsbereichs qualitativ unterschiedliche Verhaltensweisen der Asymmetrie zwischen oberen und unterem SPP beobachtet worden sind, lohnt es sich noch eine weiter Darstellungsart einzuführen:
		
		
		 In Abbildung \ref{fig:angular_int_fp} wurde die über den Radius integrierte azimutal Winkel abhängige Intensitätsverteilung im Ortsbild bei einer Orientierung des $\lambda / 4$-Plättchens von $\alpha_{\lambda/4} = 45^\circ$ und  $\alpha_{\lambda/4} = 135^\circ$ dargestellt (also links- und rechts-zirkulare Polarisation). Um das Signal zu glätten wurden dabei jeweils ein Winkelbereich von $\Delta \alpha = 2^\circ$ zu einem einzelnen Durchschnittswert zusammengefasst. Um aus diesen Daten die Azimutalwinkel abhängige Asymmetrie zwischen links- und rechts-polarisierte Anregungsstrahlung zu gewinnen wurde aus den beiden Intensitäten die relative Differenz gebildet. Diese relative Differenz ist in Abbildung \ref{fig:angular_int_diff_fp} dargestellt.
		\paragraph{Ortsraum}
		\begin{align}
			P_r(\phi, \alpha_{\lambda/4}) &= \int_{0}^{r_\mathrm{max}}r\,\mathrm{d}r I_r(r, \phi, \alpha_{\lambda /4}) \\
			D_r(\phi, \alpha_{\lambda/4}) &= \dfrac{P_r(\phi, \alpha_{\lambda/4} = 45^\circ) - P_r(\phi, \alpha_{\lambda/4} = 135^\circ)}{P_r(\phi, \alpha_{\lambda/4} = 45^\circ) + P_r(\phi, \alpha_{\lambda/4} = 135^\circ)}
		\end{align}
		\begin{figure}		
			\begin{subfigure}[b]{0.5\textwidth}
				\centering
			...
			\end{subfigure}
			\caption{Abbildung \subref{fig:angular_int_fp} zeigt die azimutal-winkelabhängige Intensitätsverteilung der Leckstrahlung eines an einem Punktdefekt angeregten SPP im Ortsraum bei links- und rechts-zirkular polarisierte Anregungsstrahlung. Abbildung \subref{fig:angular_int_diff_fp} zeigt die relative Differenz zwischen diesen beiden Intensitätsverteilungen. Die $0$-Line wurde in der Differenzdarstellung durch einen zum Ursprung konzentrischen Kreisring markiert. Für die Darstellung wurde ein Polarkoordinatensystem gewählt, um den Bezug zu den Abbildungen \ref{fig:angular_int_diff_fp} besser herzustellen. Hierbei ist zu beachten, dass die von der Kurve eingeschlossenen Flächen nicht mehr der über einen Winkelbereich integrierten Intensitätsdifferenz entsprechen, da die Kurve durch die Darstellung in Polarkoordinaten verzerrt wird.}
		...
		\begin{align}
			...
			D_k(\phi, \alpha_{\lambda/4}) &= \dfrac{P_k(\phi, \alpha_{\lambda/4} = 45^\circ) - P_k(\phi, \alpha_{\lambda/4} = 135^\circ)}{P_k(\phi, \alpha_{\lambda/4} = 45^\circ) + P_k(\phi, \alpha_{\lambda/4} = 135^\circ)}
		\end{align}
		\begin{figure}		
			\begin{subfigure}[b]{0.5\textwidth}
				\centering
